...
class SAPIENVisionDataset(data.Dataset):
    ...
    def load_data(self, data_list):
        ...
        for i in bar(range(len(data_list))):
            ...
            with open(os.path.join(cur_dir, 'result.json'), 'r') as fin:
                ...
                final_dist = np.array(result_data['final_dist'], dtype=np.float32)
                

                # load original data
                if success:
                    cur_data = (cur_dir, cur_shape_id, cur_category, cur_cnt_id, cur_trial_id, \
                            ori_pixel_ids, pixel_ids, gripper_direction_camera, gripper_forward_direction_camera, True, True, final_dist)
                    self.true_data[cur_primact_type].append(cur_data)
                    suc_num += 1
                else:
                    if not self.only_true_data:
                        cur_data = (cur_dir, cur_shape_id, cur_category, cur_cnt_id, cur_trial_id, \
                                ori_pixel_ids, pixel_ids, gripper_direction_camera, gripper_forward_direction_camera, True, False, final_dist)
                        self.false_data[cur_primact_type].append(cur_data)
                        fail_num += 1
                # load neg-direction false data
                if not self.no_neg_dir_data:
                    cur_data = (cur_dir, cur_shape_id, cur_category, cur_cnt_id, cur_trial_id, \
                            ori_pixel_ids, pixel_ids, -gripper_direction_camera, gripper_forward_direction_camera, False, False, final_dist)
                    self.false_data[cur_primact_type].append(cur_data)
                    fail_num += 1

        print('num success: ', suc_num, 'num fail: ', fail_num)
    ...
    def __getitem__(self, index):
        ...
        if self.no_true_false_equal:
            if index < len(self.false_data[primact_type]):
                cur_dir, shape_id, category, cnt_id, trial_id, ori_pixel_ids, pixel_ids, \
                        gripper_direction_camera, gripper_forward_direction_camera, is_original, result, final_dist = \
        ...
        for feat in self.data_features:
            ...
            if feat == 'rgb-d':
                with Image.open(os.path.join(cur_dir, 'rgb.png')) as fimg:
                    out1 = np.array(fimg.resize((self.img_size, self.img_size)), dtype=np.float32) / 255
                out1 = torch.from_numpy(out).permute(2, 0, 1).unsqueeze(0)
                pritn(out1)
                with Image.open(os.path.join(cur_dir, 'depth.png')) as fimg:
                    out2 = np.array(fimg.resize((self.img_size, self.img_size)), dtype=np.float32) / 255
                    print(out2.shape)
                out2 = torch.from_numpy(out).permute(2, 0, 1).unsqueeze(0)
            ...
            elif feat == 'final_dist':
                # print('final_dist', final_dist)
                # print(type(final_dist))
                final_dist = torch.from_numpy(final_dist).unsqueeze(0)
                data_feats = data_feats + (final_dist,)
