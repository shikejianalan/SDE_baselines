...
Image.fromarray((depth*255).astype(np.uint8)).save(os.path.join(out_dir, 'depth.png'))
...
# final_dist = final_dist + round(np.random.randint(0, 50)/100, 2)
final_dist = 0.05 + round(np.random.random()/10, 2)
print(final_dist)
out_info['final_dist'] = final_dist 
...
env.render()
rgb_start_pose, _ = cam.get_observation()
Image.fromarray((rgb_start_pose*255).astype(np.uint8)).save(os.path.join(out_dir, 'viz_start_pose.png'))
