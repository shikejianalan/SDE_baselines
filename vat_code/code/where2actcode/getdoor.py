...
output_dir = FLAGS.pc_path


h5_file = h5py.File(os.path.join(input_dir, f'cam_XYZA_1_1.h5'), 'r')
mask_file = Image.open(os.path.join(input_dir, f'interaction_mask_1_1.png'))
...
#PlyData([el], text=False).write("./ks-work/body1.ply")
tmp = [(pc2[i][0], pc2[i][1], pc2[i][2]) for i in range(pc2.shape[0])]
vertex = np.array(tmp, dtype=[('x', 'f4'), ('y', 'f4'), ('z', 'f4')])
el = PlyElement.describe(vertex, 'vertex', comments=['vertices'])
PlyData([el], text=False).write(output_dir)
