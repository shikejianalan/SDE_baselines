...
''' main '''
input_dir = '/home/wuruihai/where2actCode/real_data/realExp/results3/'
...
json_file = open(os.path.join(input_dir, 'wp_world_%d.json' % (way_id)), 'r')
json_content = json.load(json_file)


gripper_position = json_content['waypoints']
cont = json_content['contact_point']

tmp=[(cont[0],cont[1],cont[2])]
vertex = np.array(tmp, dtype=[('x', 'f4'), ('y', 'f4'), ('z', 'f4')])
el = PlyElement.describe(vertex, 'vertex', comments=['vertices'])
PlyData([el], text=False).write(output_dir+f'/_traj_start_{way_id}.ply')

tmp = [(gripper_position[0][0], gripper_position[0][1], gripper_position[0][2]) for i in range(len(gripper_position))]
vertex = np.array(tmp, dtype=[('x', 'f4'), ('y', 'f4'), ('z', 'f4')])
el = PlyElement.describe(vertex, 'vertex', comments=['vertices'])
PlyData([el], text=False).write(output_dir+f'/_traj_{way_id}.ply')

print(len(gripper_position))
tmp=[]
for i in range(len(gripper_position)):
    if i==0:
        x,y,z = cont[0], cont[1], cont[2]
        ex,ey,ez = gripper_position[i][0], gripper_position[i][1], gripper_position[i][2]
    else :
        x,y,z = gripper_position[i-1][0], gripper_position[i-1][1], gripper_position[i-1][2]
        ex,ey,ez = gripper_position[i][0], gripper_position[i][1], gripper_position[i][2]
    dx = ex-x
    dy = ey-y
    dz = ez-z
    for j in range(5):
        ...
        y = y + dy/5
        z = z + dz/5
        tmp.append((x+random.random()*0.01,y+random.random()*0.01,z+random.random()*0.01))
vertex = np.array(tmp, dtype=[('x', 'f4'), ('y', 'f4'), ('z', 'f4')])
el = PlyElement.describe(vertex, 'vertex', comments=['vertices'])
PlyData([el], text=False).write(output_dir+f'/_traj_dense_{way_id}.ply')
