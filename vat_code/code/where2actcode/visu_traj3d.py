...
root = '/media/wuruihai/sixt/where2act/data/gt_data-multiviews-allCat-pushing0p05_TD3_curiosityDriven/allShape_52302/NOVEL_EVAL0p0_th0p6/allShape_StorageFurniture_52302_0'
...
all_waypoints=[]
all_waypoints_color=[]
out_dir = os.path.join(root, 'traj_3d')

h5_file = h5py.File(os.path.join(root, f'cam_XYZA_{way_id}.h5'), 'r')
mask_file = Image.open(os.path.join(root, f'interaction_mask_{way_id}.png'))
...
pc_color = np.zeros(pc.shape[0]) + mask * 0.3   # blue


if not os.path.exists(out_dir):
    os.mkdir(out_dir)
json_file = open(os.path.join(root, 'result_%d.json' % (way_id)), 'r')
...
for wp_idx in range(gripper_position.shape[0]):
    ...
    all_waypoints_color.append(1)
    
all_waypoints = np.array(all_waypoints)
all_waypoints_color = np.array(all_waypoints_color)
out_fn = os.path.join(out_dir, 'traj_%d' % (way_id))
utils.render_pts_label_png(out_fn, np.concatenate([pc, all_waypoints]), np.concatenate([pc_color, all_waypoints_color]))
