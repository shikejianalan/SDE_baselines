...
def train(conf, train_shape_list, train_data_list, val_data_list, all_train_data_list):
    ...
    data_features = ['pcs', 'pc_pxids', 'pc_movables', 'gripper_img_target', 'gripper_direction_camera', 'gripper_forward_direction_camera', \
            'result', 'cur_dir', 'shape_id', 'trial_id', 'is_original', 'final_dist']
        ...
        for train_batch_ind, batch in train_batches:
            ...
            if conf.sample_succ:
                ...
                with torch.no_grad():
                    ...
                    random_dist = torch.from_numpy(0.05 + np.random.random((conf.batch_size, 1))/10).float().to(conf.device)

                    # test over the entire image
                    whole_pc_scores1 = network.inference_whole_pc(whole_feats, random_dirs1, random_dirs2, random_dist)     # B x N
                    whole_pc_scores2 = network.inference_whole_pc(whole_feats, -random_dirs1, random_dirs2, random_dist)     # B x N
...
def forward(batch, data_features, network, conf, \
    ...
    final_dist = torch.cat(batch[data_features.index('final_dist')], dim=0).to(conf.device).unsqueeze(1)
    # forward through the network
    pred_result_logits, pred_whole_feats = network(input_pcs, input_dirs1, input_dirs2, final_dist)     # B x 2, B x F x N
