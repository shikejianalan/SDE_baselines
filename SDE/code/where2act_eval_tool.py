...
class Evaluator:
    ...
    def network_infer(self, network):
        ...
        # result = torch.from_numpy(np.append([position_cam, self.pose]))
    ...
    def test_contact_once(self, object_link_id, robot_first_time):
        ...
        if self.conf.primact_type == 'pulling':
            try:
                ...
                try:
                    ...
                    succ_imgs = self.robot.move_to_target_pose_visu(end_rotmat, 2000, vis_gif=self.conf.vis_gif, cam=self.cam)
                ...
                if not (success_grasp and init_success):
                    ...
                    try:
                        succ_imgs = self.robot.move_to_target_pose_visu(final_rotmat, 2000, vis_gif=self.conf.vis_gif, cam=self.cam)
                        self.robot.wait_n_steps(2000)
                    except Exception:
                        print("fail")
                        success = False
        ...
        return {"success": success, "succ_imgs": succ_imgs}
    
    def test_contact(self, network, shape_id_list, camera_angle_list, articu_angle_list, save_dir):
        succ_time = 0
        total_time = 0
