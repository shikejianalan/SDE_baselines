...
class Actor(nn.Module):
    def __init__(self, feat_dim, rv_dim):
        ...
        self.mlp2 = nn.Linear(feat_dim, 3+3+1)
    ...
    def forward(self, pixel_feats, rvs):
        ...
        net = self.mlp2(net)
        # print('net_dim', net.shape)
        net_act, net_dist = net[:,:-1], net[:,-1].unsqueeze(1)
        # print('net_dim', net_act.shape, net_dist.shape)
        # print(net_dist.shape)

        net_act = net_act.reshape(-1, 3, 2)
        net_act = self.bgs(net_act)[:, :, :2].reshape(-1, 6)
        return net_act, net_dist
...
class Network(nn.Module):
    ...
    def forward(self, pcs, dirs1, dirs2, gt_result, final_dist):
        ...
        input_queries = torch.cat([dirs1, dirs2, final_dist], dim=1)
        ...
        expanded_pred_s6d, pred_dist = self.actor(expanded_net, expanded_rvs)
        expanded_input_s6d = input_s6d.unsqueeze(dim=1).repeat(1, self.rv_cnt, 1).reshape(batch_size*self.rv_cnt, -1)
        expanded_input_dist = final_dist.unsqueeze(dim=1).repeat(1, self.rv_cnt, 1).reshape(batch_size*self.rv_cnt, -1)
        # print(expanded_input_dist.shape)
        # print(pred_dist.shape)
        expanded_actor_coverage_loss_per_rv = self.actor.get_6d_rot_loss(expanded_pred_s6d, expanded_input_s6d)

        # assert expanded_actor_coverage_loss_per_rv.shape == pred_dist.shape
        # dist_loss = pred_dist - expanded_input_dist
        # print(dist_loss.squeeze().shape)

        loss_function = nn.MSELoss()
        dist_loss = loss_function(pred_dist , expanded_input_dist)
        expanded_actor_coverage_loss_per_rv += dist_loss
        ...
        with torch.no_grad():
            expanded_queries = torch.cat([expanded_pred_s6d, pred_dist], dim=1)
    ...
    # for sample_succ
    def inference_whole_pc(self, feats, dirs1, dirs2, final_dist):
    ...
    def inference_actor(self, pcs):
        ...
        expanded_net = net.unsqueeze(dim=1).repeat(1, self.rv_cnt, 1).reshape(batch_size*self.rv_cnt, -1)
        expanded_rvs = rvs.reshape(batch_size*self.rv_cnt, -1)
        expanded_pred_s6d, expanded_pred_dist = self.actor(expanded_net, expanded_rvs)
        pred_s6d = expanded_pred_s6d.reshape(batch_size, self.rv_cnt, 6)
        pred_dist = expanded_pred_dist.reshape(batch_size, self.rv_cnt, 1)

        return pred_s6d, pred_dist
    
    def inference_critic(self, pcs, dirs1, dirs2, final_dist, abs_val=False):
        pcs = pcs.repeat(1, 1, 2)
        whole_feats = self.pointnet2(pcs)
        net = whole_feats[:, :, 0]
