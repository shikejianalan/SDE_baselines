...
rv_dim = 10
rv_cnt = 100
network = AssembleModel(eval_conf)
network.load_state_dict(torch.load(eval_conf.model_path))
...
cam = Camera(env, phi=np.pi/5, theta=np.pi/3 *2.3)
#cam = Camera(env)
...
val_batch_size = pc.size(0)
val_num_point = pc.size(1)
feats = network.get_whole_feats(pc)
...
with torch.no_grad():
    ...
    pred_action_score_map = network.get_affordance(feats)["x_states"][-1].reshape(val_batch_size, val_num_point)[0]
    pred_action_score_map *= torch.tensor(pc_movable, device=virtual_conf.device)
    render_img = affordance_render(virtual_conf, pc[0], pred_action_score_map, view=[6., 0, 270], radius=0.0075)["np_image"]
    imageio.imwrite("./output/test_" + eval_conf.shape_id + ".png", np.rot90(render_img))
    """
    ...
    print("pred_action_score_map size", pred_action_score_map.size())
    angle_x = [0, 90, 180, 270]
    angle_y = [0, 20, 40, 60, 80, 100, 120, 140, 180, 200, 220, 240, 260, 280, 300, 320, 340]
    angle_z = [0, 20, 40, 60, 80, 100, 120, 140, 180, 200, 220, 240, 260, 280, 300, 320, 340]
    """
    """
    a_x=90
    a_y=0
    a_z=270
    affordance_render(virtual_conf, pc[0], pred_action_score_map, view=[a_x, a_y, a_z], out_fn="./output/new_afford_test{}_{}_{}.png".format(a_x, a_y, a_z))
    a_x=90
    a_y=0
    a_z=90
    affordance_render(virtual_conf, pc[0], pred_action_score_map, view=[a_x, a_y, a_z], out_fn="./output/new_afford_test{}_{}_{}.png".format(a_x, a_y, a_z))
    ...
    for a_y in angle_y:
        for a_z in angle_z:
            affordance_render(virtual_conf, pc[0], pred_action_score_map, view=[2.7, a_y, a_z], radius=0.01, out_fn="./output_angle/afford_test{}_{}_{}.png".format(2.7, a_y, a_z)) # we need to adjust camera angle
    """
    #utils.export_pts_color_obj("./output/afford_test",  pc[0].cpu().numpy(), pccolors)
    #utils.export_pts_color_pts("./output/afford_test",  pc[0].cpu().numpy(), pccolors)
    exit()
