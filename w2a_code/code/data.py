    ...
    def __getitem__(self, index):
        ...
        for feat in self.data_features:
            if feat == 'img':
                ...
                out1 = torch.from_numpy(out1).permute(2, 0, 1)
                # print(out1.shape)
                with Image.open(os.path.join(cur_dir, 'depth.png')) as fimgd:
                    out2 = np.array(fimgd.resize((self.img_size, self.img_size)), dtype=np.float32) / 255
                out2 = torch.from_numpy(out2).unsqueeze(2).permute(2, 0, 1)
                # print(out2.shape)
                rgbd = torch.vstack((out1, out2)).unsqueeze(0)
                # print(rgbd.shape)
                data_feats = data_feats + (rgbd,)
            elif feat == 'pcs':
